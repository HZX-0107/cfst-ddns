# CFST-DDNS: Cloudflare 优选 IP 自动 DDNS 工具

**cfst-ddns** 是一个基于 Go 语言开发的高性能工具，它集成了 [CloudflareSpeedTest](https://github.com/XIU2/CloudflareSpeedTest) 的测速能力和腾讯云 (DNSPod) 的 API。

该工具会自动寻找当前网络环境下速度最快的 Cloudflare IP 地址，并将其自动更新到你的 DNSPod 域名解析记录中。支持 IPv4 和 IPv6。

## ✨ 主要特性

- **全自动流程**：一键运行，自动完成测速 -> 选优 -> 解析更新。
- **零依赖部署**：利用 Go 的 `embed` 特性，可将 CloudflareSpeedTest 二进制文件直接打包进程序，生成单一可执行文件，无需携带琐碎的资源文件夹。
- **双栈支持**：完美支持 IPv4 (A 记录) 和 IPv6 (AAAA 记录) 的优选与更新。
- **智能更新**：自动检测 DNS 记录是否存在，不存在则创建，存在则更新；若 IP 无变化则跳过，节省 API 调用。
- **Docker 支持**：提供 Dockerfile，支持动态版本拉取构建，轻松集成到容器环境。
- **配置灵活**：支持 YAML 配置文件，清晰管理密钥和测速参数。

## 🛠️ 准备工作

在开始之前，请确保你拥有：

1. **腾讯云 API 密钥** (SecretId, SecretKey)：用于操作 DNSPod。如果使用docker镜像，那么不需要准备以下两项
2. **CloudflareSpeedTest 二进制文件**：
   - **开发模式**：需下载对应的 `cfst` 文件并解压放入 `assets/` 目录。
   - **运行模式**：如果使用预编译好的单文件版本，则无需准备此项。
3. **IP 地址库** (`ip.txt` / `ipv6.txt`)：用于测速的 IP 段列表（放入 `assets/` 目录）。一般下载的CloudflareSpeedTest 项目文件夹中有

## 🚀 快速开始 (本地运行)

### 1. 编译

```
# 确保 assets 目录下有 cfst 二进制文件和 ip.txt
# Windows 下建议将 cfst.exe 重命名为 cfst 以适配 embed
go build -o cfst-ddns cmd/app/main.go
```

### 2. 配置

在 `configs` 目录下创建 `config.yaml` 文件（可参考下文配置说明）。

### 3. 运行

```
./cfst-ddns
```

程序启动后会自动释放内部集成的测速工具到临时目录，执行测速，并更新 DNS。

## ⚙️ 配置文件说明

文件路径：`configs/config.yaml`

```
app:
  debug: false          # 开启后，会打印详细的debug信息，调试时可以使用
  # cron: "0 */1 * * *" # 定时任务表达式 (预留功能)

tencent:
  secret_id: "你的_SecretId"
  secret_key: "你的_SecretKey"

domain:
  main_domain: "example.com" # 主域名
  sub_domain: "cf"           # 子域名 (例如 cf.example.com)
  
# 如果使用docker镜像，那么不需要配置下面路径
speedtest:
  # 测速工具路径 (如果使用 embed 模式，此路径会被程序自动覆盖为临时路径)
  bin_path: "./assets/cfst"
  
  # IP 库文件路径
  ip_file: "./assets/ip.txt"
  ipv6_file: "./assets/ipv6.txt" # 如果不需要 IPv6，可留空或删除文件
  
  # 结果输出路径
  output_csv_v4: "./assets/result_v4.csv"
  output_csv_v6: "./assets/result_v6.csv"
  
  # 测速参数
  max_ping: 9999    # 最大延迟 (毫秒)
  test_count: 500   # 测速数量
  
  # [新增] 自定义下载测速地址
  # 默认地址 speed.cloudflare.com 在某些地区可能无法访问
  # 备选地址 1: https://img.131213.xyz/Test/100m.jpg
  # 备选地址 2: https://cdn.cloudflare.steamstatic.com/steam/apps/5952/movie_max.webm
  # 备选地址 3: https://speed.cloudflare.com/__down?bytes=20000000 (默认)
  download_url: "https://cdn.cloudflare.steamstatic.com/steam/apps/5952/movie_max.webm"
```

## 🐳 Docker 部署

本项目支持多种 Docker 部署方式。推荐使用 GitHub 镜像站 (GHCR) 直接拉取。

注意使用docker部署方式，首次启动会自动生成一个config.yml文件保证启动成功，请修改此文件为自己的配置后重新启动
### 方式一：Docker Compose (推荐)

```yml
services:
  cfst-ddns:
    image: ghcr.io/hzx-0107/cfst-ddns:latest
    container_name: cfst-ddns
    # 设置时区
    environment:
      TZ: Asia/Shanghai
    volumes:
      - ./configs:/app/configs
      - ./assets:/app/assets
      - ./logs:/app/logs
    # 如果程序只是运行一次就退出(Job类型)，不需要 restart: always
    # 如果未来增加了 Cron 功能长期运行，可以开启 restart
    restart: no 
```

启动服务：

```shell
docker-compose up -d
```

### 方式二：Docker CLI

```shell
docker run -d \
  --name cfst-ddns \
  -v $(pwd)/configs:/app/configs \
  -v $(pwd)/assets:/app/assets \
  ghcr.io/hzx-0107/cfst-ddns:latest
```

### 方式三：手动构建镜像

```shell
# 默认构建
docker build -t cfst-ddns .

# 指定 CloudflareSpeedTest 版本构建
docker build --build-arg CFST_VERSION=v2.3.4 -t cfst-ddns .
```

### 运行容器

建议挂载配置目录和资源目录，以便持久化数据和修改配置。

```shell
docker run -d \
  --name cfst-ddns \
  -v $(pwd)/configs:/app/configs \
  -v $(pwd)/assets:/app/assets \
  cfst-ddns
```

**注意**：容器运行完毕后会自动退出（因为是单次任务，定时功能暂未开发好）。如果你需要定时运行，可以使用系统的 crontab 定时启动容器，或者使用 Kubernetes 的 CronJob。

## 📂 项目结构

```
cfst-ddns/
├── assets/                   # 静态资源 (Embed 目标目录)
│   ├── cfst                  # [必需] CloudflareSpeedTest 二进制
│   ├── ip.txt                # [必需] IPv4 段
│   ├── ipv6.txt              # [可选] IPv6 段
│   └── embed.go              # Go Embed 声明文件
│   └── config-example.yaml   # 构建时使用，生成config.yml保证首次启动成功
├── cmd/
│   └── app/
│       └── main.go     # 程序入口
├── configs/            # 配置文件目录
│   └── config-example.yaml
├── internal/           # 核心业务逻辑
│   ├── config/         # 配置加载
│   ├── dns/            # 腾讯云 SDK 封装
│   └── speedtest/      # 测速工具调用与解析
├── Dockerfile          # Docker 构建文件
└── go.mod              # Go 依赖管理
```

## ⚠️ 免责声明

本项目仅供学习交流使用。使用 CloudflareSpeedTest 进行优选 IP 可能会受限于网络环境，请遵守当地法律法规和网络运营商的服务条款。

## 🙏 致谢

- [XIU2/CloudflareSpeedTest](https://github.com/XIU2/CloudflareSpeedTest): 强大的 Cloudflare 优选 IP 工具。
- [Tencent Cloud SDK for Go](https://github.com/tencentcloud/tencentcloud-sdk-go): 腾讯云官方 Go SDK。